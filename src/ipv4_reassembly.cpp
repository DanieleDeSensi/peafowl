...
#define debug_print(fmt, ...)                                          \
  do {                                                                 \
    if (DPI_DEBUG_FRAGMENTATION_v4) fprintf(stderr, fmt, __VA_ARGS__); \
  } while (0)

#define DPI_IP_FRAGMENTATION_MAX_DATAGRAM_SIZE 65535
#define DPI_IPv4_FRAGMENTATION_MINIMUM_MTU 576

typedef struct dpi_ipv4_fragmentation_flow dpi_ipv4_fragmentation_flow_t;
typedef struct dpi_ipv4_fragmentation_source dpi_ipv4_fragmentation_source_t;
...
dpi_ipv4_fragmentation_state_t* dpi_reordering_enable_ipv4_fragmentation(
    uint16_t table_size) {
  dpi_ipv4_fragmentation_state_t* r = (dpi_ipv4_fragmentation_state_t*)calloc(
      1, sizeof(dpi_ipv4_fragmentation_state_t));
  if (unlikely(r == NULL)) {
    free(r);
    return NULL;
  }
  r->table_size = table_size;
  r->table = (dpi_ipv4_fragmentation_source_t**)malloc(
      table_size * sizeof(dpi_ipv4_fragmentation_source_t*));
  ...
  r->total_memory_limit = DPI_IPv4_FRAGMENTATION_DEFAULT_TOTAL_MEMORY_LIMIT;
  r->timeout = DPI_IPv4_FRAGMENTATION_DEFAULT_REASSEMBLY_TIMEOUT;
  r->total_used_mem = 0;
#if DPI_THREAD_SAFETY_ENABLED == 1
    ...
    for (i = 0; i < frag_state->table_size; i++) {
      if (frag_state->table[i]) {
        ...
        while (source) {
          ...
          dpi_ipv4_fragmentation_delete_source(frag_state, source);
          source = tmp_source;
        }
      }
    }
...
static
    ...
    uint16_t
    dpi_ipv4_fragmentation_hash_function(dpi_ipv4_fragmentation_state_t* state,
                                         uint32_t src_ip) {
...
#endif
    void
    dpi_ipv4_fragmentation_delete_flow(dpi_ipv4_fragmentation_state_t* state,
...
static
  ...
  uint16_t hash_index = dpi_ipv4_fragmentation_hash_function(state, addr);
  ...
  source = (dpi_ipv4_fragmentation_source_t*)malloc(
      sizeof(dpi_ipv4_fragmentation_source_t));
...
static
#endif
    dpi_ipv4_fragmentation_flow_t*
    dpi_ipv4_fragmentation_find_or_create_flow(
        dpi_ipv4_fragmentation_state_t* state,
        dpi_ipv4_fragmentation_source_t* source, const struct iphdr* iph,
  ...
  flow = (dpi_ipv4_fragmentation_flow_t*)malloc(
      sizeof(dpi_ipv4_fragmentation_flow_t));
...
#endif
    unsigned char*
    dpi_ipv4_fragmentation_build_complete_datagram(
        dpi_ipv4_fragmentation_state_t* state,
        dpi_ipv4_fragmentation_flow_t* flow) {
 ...
 */
...
#endif
  source = dpi_ipv4_fragmentation_find_or_create_source(state, iph->saddr);
  if (unlikely(source == NULL)) {
  ...
  while ((state->timer_head) &&
    ...
    dpi_ipv4_fragmentation_source_t* tmpsource =
        ((dpi_ipv4_fragmentation_flow_t*)state->timer_head->data)->source;
    dpi_ipv4_fragmentation_delete_flow(
        state, (dpi_ipv4_fragmentation_flow_t*)state->timer_head->data);
    if (source->flows == NULL) {
...
* fragment is useless.
  ...
  dpi_reassembly_insert_fragment(&(flow->fragments), data + ihl, offset, end,
