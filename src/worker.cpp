...
#endif
    void
    dpi_free_task(mc_dpi_task_t* task) {
#if DPI_NUMA_AWARE
...
int dpi_L3_L4_emitter::svc_init() {
  worker_debug_print(
      "[worker.cpp]: L3_L4 emitter mapped on "
      "processor: %d\n",
      proc_id);
...
void dpi_L3_L4_emitter::notifyRethreading(size_t oldNumWorkers,
...
dpi_L3_L4_worker::~dpi_L3_L4_worker() { free(in); }

#ifdef ENABLE_RECONFIGURATION
void dpi_L3_L4_worker::notifyRethreading(size_t oldNumWorkers,
                                         size_t newNumWorkers) {
  ...
  worker_debug_print(
      "[worker.cpp]: L3_L4 worker. v4_worker_table_size: %d "
...
int dpi_L3_L4_worker::svc_init() {
  worker_debug_print(
      "[worker.cpp]: L3_L4 worker %d mapped "
      "on processor: %d\n",
      worker_id, proc_id);
  ff_mapThreadToCpu(proc_id, -20);
  return 0;
...
void* dpi_L3_L4_worker::svc(void* task) {
  ...
  memcpy(in, real_task->input_output_task_t.L3_L4_input_task_t,
         DPI_MULTICORE_DEFAULT_GRAIN_SIZE * sizeof(L3_L4_input_task_struct));
  ...
  for (uint i = 0; i < DPI_MULTICORE_DEFAULT_GRAIN_SIZE; i++) {
    ...
    real_task->input_output_task_t.L3_L4_output_task_t[i].status =
        mc_dpi_extract_packet_infos(this->state, in[i].pkt, in[i].length,
      ...
      if (likely(real_task->input_output_task_t.L3_L4_output_task_t[i].status !=
        ...
        if (pkt_infos.ip_version == DPI_IP_VERSION_4) {
          real_task->input_output_task_t.L3_L4_output_task_t[i].hash_result =
              dpi_compute_v4_hash_function((dpi_flow_DB_v4*)this->state->db4,
                                           &pkt_infos);
          ...
          real_task->input_output_task_t.L3_L4_output_task_t[i].hash_result =
              dpi_compute_v6_hash_function((dpi_flow_DB_v6*)this->state->db6,
                                           &pkt_infos);
          real_task->input_output_task_t.L3_L4_output_task_t[i]
              .destination_worker =
              real_task->input_output_task_t.L3_L4_output_task_t[i]
...
dpi_L3_L4_collector::dpi_L3_L4_collector(uint16_t proc_id) : proc_id(proc_id) {
  ;
}

int dpi_L3_L4_collector::svc_init() {
  worker_debug_print(
      "[worker.cpp]: L3_L4 collector mapped "
      "on processor: %u\n",
      proc_id);
  ff_mapThreadToCpu(proc_id, -20);
  return 0;
...
void* dpi_L3_L4_collector::svc(void* task) { return task; }
  ...
  for (uint i = 0; i < DPI_MULTICORE_DEFAULT_GRAIN_SIZE; i++) {
    ...
    uint16_t destination_worker =
        real_task->input_output_task_t.L3_L4_output_task_t[i]
            .destination_worker;
    worker_debug_print(
        "[worker.cpp]: L7 emitter: Inserted"
    ...
    if (pfs + 1 == DPI_MULTICORE_DEFAULT_GRAIN_SIZE) {
      ...
      out->input_output_task_t
          .L3_L4_output_task_t[DPI_MULTICORE_DEFAULT_GRAIN_SIZE - 1] =
          real_task->input_output_task_t.L3_L4_output_task_t[i];
...
int dpi_L7_worker::svc_init() {
  worker_debug_print(
      "[worker.cpp]: L7 worker %u mapped on"
      " processor: %u. Tid: %lu\n",
...
void* dpi_L7_worker::svc(void* task) {
  ...
  memcpy(temp, real_task->input_output_task_t.L3_L4_output_task_t,
         DPI_MULTICORE_DEFAULT_GRAIN_SIZE * sizeof(L3_L4_output_task_struct));
  ...
  for (uint i = 0; i < DPI_MULTICORE_DEFAULT_GRAIN_SIZE; i++) {
    ...
    if (unlikely(flow_infos == NULL)) {
      ...
      real_task->input_output_task_t.L7_output_task_t[i].result =
          dpi_stateless_get_app_protocol(state, flow_infos, &(infos));
      if (real_task->input_output_task_t.L7_output_task_t[i].result.status ==
          DPI_STATUS_TCP_CONNECTION_TERMINATED) {
        if (ipv4_flow != NULL) {
          mc_dpi_flow_table_delete_flow_v4((dpi_flow_DB_v4_t*)state->db4,
                                           state->flow_cleaner_callback,
                                           this->worker_id, ipv4_flow);
        } else {
          mc_dpi_flow_table_delete_flow_v6((dpi_flow_DB_v6_t*)state->db6,
                                           state->flow_cleaner_callback,
...
dpi_collapsed_emitter::dpi_collapsed_emitter(
    mc_dpi_packet_reading_callback** cb, void** user_data, uint8_t* terminating,
    ff::SWSR_Ptr_Buffer* tasks_pool, dpi_library_state_t* state,
    uint16_t num_L7_workers, uint32_t v4_table_size, uint32_t v6_table_size,
    dpi_L7_scheduler* lb, uint16_t proc_id)
    : dpi_L7_emitter(lb, num_L7_workers, proc_id), proc_id(proc_id) {
  L3_L4_emitter = new dpi::dpi_L3_L4_emitter(state, cb, user_data, terminating,
                                             proc_id, tasks_pool);
  L3_L4_worker = new dpi::dpi_L3_L4_worker(state, 0, num_L7_workers, proc_id,
...
#ifdef ENABLE_RECONFIGURATION
void dpi_collapsed_emitter::notifyRethreading(size_t oldNumWorkers,
                                              size_t newNumWorkers) {
...
int dpi_collapsed_emitter::svc_init() {
  ...
  worker_debug_print(
      "[worker.cpp]: collapsed emitter mapped "
      "on processor: %d\n",
      proc_id);
  return 0;
}
