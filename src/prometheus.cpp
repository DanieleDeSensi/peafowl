...
static bool pfwl_prometheus_create_labels(pfwl_pkt_infos_t& infos,
  ...
  labels.insert(
      pair<string, string>("srcport", to_string(ntohs(infos.srcport))));
  labels.insert(
      pair<string, string>("dstport", to_string(ntohs(infos.dstport))));
  labels.insert(pair<string, string>("l4prot", to_string(infos.l4prot)));
  // labels.insert(pair<string, string>("direction",
  // to_string(infos.direction))); If we want to use direction, we need to have
  // two different counters (one for direction 0 and one for direction 1, and we
  // need to increment only the counter correpsonding to the pkt direction)
  return true;
}
