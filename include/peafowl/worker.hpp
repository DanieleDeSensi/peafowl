...
#define worker_debug_print(fmt, ...)                            \
  do {                                                          \
    if (DPI_DEBUG_MP_WORKER) fprintf(stdout, fmt, __VA_ARGS__); \
  } while (0)
...
typedef struct mc_dpi_task {
  union input_output_task {
    ...
    L7_output_task_struct L7_output_task_t[DPI_MULTICORE_DEFAULT_GRAIN_SIZE];
  } input_output_task_t;
  char padding[DPI_CACHE_LINES_PADDING_REQUIRED(sizeof(input_output_task_t))];
} mc_dpi_task_t;
...
class dpi_L3_L4_emitter : public ffnode {
 ...
 public:
  dpi_L3_L4_emitter(dpi_library_state_t* state,
                    mc_dpi_packet_reading_callback** cb, void** user_data,
...
class dpi_L3_L4_worker : public ffnode {
 ...
 public:
  dpi_L3_L4_worker(dpi_library_state_t* state, uint16_t worker_id,
...
class dpi_L7_emitter : public ffnode {
 ...
 public:
  dpi_L7_emitter(dpi_L7_scheduler* lb, uint16_t num_L7_workers,
                 uint16_t proc_id);
  ~dpi_L7_emitter();
  int svc_init();
  void* svc(void* task);
...
class dpi_L7_worker : public ffnode {
 ...
 public:
  dpi_L7_worker(dpi_library_state_t* state, uint16_t worker_id,
                uint16_t proc_id);
  ~dpi_L7_worker();
...
class dpi_collapsed_emitter : public dpi::dpi_L7_emitter {
 ...
 public:
  dpi_collapsed_emitter(mc_dpi_packet_reading_callback** cb, void** user_data,
                        uint8_t* terminating, ff::SWSR_Ptr_Buffer* tasks_pool,
                        dpi_library_state_t* state, uint16_t num_L7_workers,
                        uint32_t v4_table_size, uint32_t v6_table_size,
