...
#define debug_print(fmt, ...)                                          \
  do {                                                                 \
    if (DPI_DEBUG_FRAGMENTATION_v6) fprintf(stderr, fmt, __VA_ARGS__); \
  } while (0)

#define DPI_IP_FRAGMENTATION_MAX_DATAGRAM_SIZE 65535
#define DPI_IPv6_FRAGMENTATION_MINIMUM_MTU 1280

typedef struct dpi_ipv6_fragmentation_flow dpi_ipv6_fragmentation_flow_t;
typedef struct dpi_ipv6_fragmentation_source dpi_ipv6_fragmentation_source_t;
...
#endif
    void
    dpi_ipv6_fragmentation_delete_source(
...
dpi_ipv6_fragmentation_state_t* dpi_reordering_enable_ipv6_fragmentation(
    uint16_t table_size) {
  dpi_ipv6_fragmentation_state_t* r = (dpi_ipv6_fragmentation_state_t*)calloc(
      1, sizeof(dpi_ipv6_fragmentation_state_t));
  if (r == NULL) {
    return NULL;
  }
  r->table_size = table_size;
  r->table = (dpi_ipv6_fragmentation_source_t**)malloc(
      table_size * sizeof(dpi_ipv6_fragmentation_source_t*));
  if (r->table == NULL) {
    free(r);
    return NULL;
  }
  ...
  r->total_memory_limit = DPI_IPv6_FRAGMENTATION_DEFAULT_TOTAL_MEMORY_LIMIT;
  r->timeout = DPI_IPv6_FRAGMENTATION_DEFAULT_REASSEMBLY_TIMEOUT;
  r->total_used_mem = 0;
#if DPI_THREAD_SAFETY_ENABLED == 1
    ...
    for (i = 0; i < frag_state->table_size; i++) {
      if (frag_state->table[i]) {
        ...
        while (source) {
          ...
          dpi_ipv6_fragmentation_delete_source(frag_state, source);
          source = tmp_source;
        }
      }
    }
...
static
    ...
    /**  Shift-Add-XOR hash. **/
    uint16_t
    dpi_ipv6_fragmentation_hash_function(dpi_ipv6_fragmentation_state_t* state,
                                         struct in6_addr addr) {
  uint16_t h = 0;
  uint8_t i;

  for (i = 0; i < 16; i++) h ^= (h << 5) + (h >> 2) + addr.s6_addr[i];
...
static
  ...
  uint16_t hash_index = dpi_ipv6_fragmentation_hash_function(state, addr);
  ...
  source = (dpi_ipv6_fragmentation_source_t*)malloc(
      sizeof(dpi_ipv6_fragmentation_source_t));
...
static
#endif
    void
    dpi_ipv6_fragmentation_delete_flow(dpi_ipv6_fragmentation_state_t* state,
  ...
  dpi_reassembly_delete_timer(&(state->timer_head), &(state->timer_tail),
                              &(flow->timer));
  ...
  if (flow->prev == NULL) {
    ...
    if (source->flows != NULL) source->flows->prev = NULL;
  } else {
    flow->prev->next = flow->next;
    if (flow->next) flow->next->prev = flow->prev;
  }
  free(flow);
}
...
static
#endif
    dpi_ipv6_fragmentation_flow_t*
    dpi_ipv6_fragmentation_find_or_create_flow(
        dpi_ipv6_fragmentation_state_t* state,
        dpi_ipv6_fragmentation_source_t* source, uint32_t id,
  ...
  for (flow = source->flows; flow != NULL; flow = flow->next) {
     ...
     **/
    if (id == flow->id && dpi_v6_addresses_equal(dstaddr, flow->dstaddr)) {
      return flow;
    }
  }

  /** Not found, create a new flow. **/
  flow = (dpi_ipv6_fragmentation_flow_t*)malloc(
      sizeof(dpi_ipv6_fragmentation_flow_t));
  ...
  if (flow->next) flow->next->prev = flow;
  source->flows = flow;
  /* Set the timer. */
...
static
#endif
    unsigned char*
    dpi_ipv6_fragmentation_build_complete_datagram(
        dpi_ipv6_fragmentation_state_t* state,
        dpi_ipv6_fragmentation_flow_t* flow) {
  ...
  count = dpi_reassembly_ip_compact_fragments(flow->fragments, &pkt_data, len);
 ...
 */
...
#endif
  source = dpi_ipv6_fragmentation_find_or_create_source(state, ip6->ip6_src);
  ...
  while ((state->timer_head) &&
    ...
    dpi_ipv6_fragmentation_source_t* tmpsource =
        ((dpi_ipv6_fragmentation_flow_t*)state->timer_head->data)->source;
    dpi_ipv6_fragmentation_delete_flow(
        state, (dpi_ipv6_fragmentation_flow_t*)state->timer_head->data);
    if (source->flows == NULL) {
  ...
  /* Find the flow. */
  flow = dpi_ipv6_fragmentation_find_or_create_flow(
  ...
  dpi_reassembly_insert_fragment(&(flow->fragments), fragmentable_start, offset,
   ...
   **/
  if (flow->len != 0 &&
      dpi_reassembly_ip_check_train_of_contiguous_fragments(flow->fragments)) {
    unsigned char* r;
    debug_print("%s\n",
